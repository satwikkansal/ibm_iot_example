...
So it helps to add a layer between devices and cloud where these heterogeneous protocols and data-formats can be aggregated before sending them over to the cloud. This layer can be called Gateway or Edge layer, and it could be integrated into your router or on another dedicated device altogether. Apart from aggregation, the Gateway / Edge layer can also serve the following purposes,
...
We'll be using a single smartphone device in our project implementation. But we'd like to design the system in a way that's easy to scale to multiple devices in the future and touch upon different concepts like security, device management, and analytics. We'll add another device to the system as a gateway. I have an old Android smartphone to use as a device and a spare raspberry-pi that supports wifi. You can also use a different microcontroller or even your PC to run the code related to the gateway. We'll be leveraging IBM cloud's IoT platform and other services to avoid reinventing the wheel for things like security, device-management, monitoring, visualizations, etc.

## The IoT protocols

As mentioned earlier, the IoT devices may be heterogeneous and use different protocols to communicate with the gateway. The most prevalent protocol however, in the IoT world is MQTT (MQ Telemetry Transport). The "MQ" in "MQTT" comes from IBM's [MQ](https://www.ibm.com/in-en/products/mq) product line. 

In short, MQTT is a machine to machine messaging protocol. It is designed to be lightweight and fast, keeping in mind the power and resource constraints of a typical IoT device. The protocol runs over TCP-IP and operates on the publish-subscribe model. The key entities in a functional MQTT system are,

- MQTT client
- Message broker
- Topics and messages

The clients communicate only with the broker (not with each other), where they can publish messages or subscribe messages to hierarchical message topics. Topics are used to segregate different data. You can read more about how MQTT in the context of the IBM IoT Platform in the [Getting to know MQTT](https://developer.ibm.com/articles/iot-mqtt-why-good-for-iot/) article.
...
As we'll be covering MQTT protocol for the gateway to cloud communication, and HTTP is un-necessary overhead for continuous telemetry data, I'm picking [SensorUDP](https://play.google.com/store/apps/details?id=com.ubccapstone.sensorUDP&hl=en_IN) to stream the sensor data for this post.
...
So I've created a gateway type called "raspi" and registered a gateway device with ID "raspi-1". By default, the gateway devices are registered with a privileged permission role that gives them the authority to add devices to the Platform Service.

### 3. Create another device type for the smartphone
...
**Note:** We don't need to register our smartphone device (just like we registered our raspi-1) for now. As you'll see later, we'll use the gateway (`raspi` device type) to register the smartphone programmatically.

### 4. Configure and connect the gateway to the cloud

The gateway device (raspberry-pi) would be communicating with the cloud via the MQTT protocol. We'll need the following pieces of information (that you copied earlier) to configure the connection to MQTT broker in IBM IoT cloud,
